...
//! `UserId` *is* the iroh `EndpointId`, so a box sealed to a member opens for the
...
use lait::crypto::{open_sealed, random_key, seal_to, user_from_seed};
use lait::ids::UserId;
...
fn iroh_keypair_is_the_same_ed25519_pair_as_a_lait_seed() {
    ...
    let from_iroh = UserId::from_key_string(sk.public().to_string());
    ...
    assert_eq!(
        from_iroh,
        user_from_seed(&seed),
        "iroh's identity must equal lait's seed-derived UserId"
    );
    assert_eq!(sk.to_bytes(), seed, "iroh's secret bytes must be the seed");

    // And a box sealed to that UserId opens for the holder of the seed.
    let key = random_key();
    let sealed = seal_to(&from_iroh, &key).expect("seal to iroh-derived user");
    assert_eq!(
        open_sealed(&seed, &from_iroh, &sealed).as_deref(),
        Some(&key[..]),
        "an iroh-keyed member must open a box sealed to their UserId"
    );
}
